(function($) {
  $.fn.tokenField = function(options) {
    ...
    function inputHtml(name) {
         return "<div class='token-input'><input type='text' style='border: none !important;'/><span class='token-input-sizer'>###</span></div>";
    }
    ...
    function tokenHtml(name,text) {
      return "<a href='#' class='token'><span><span><span><span>"
              + text
              + (settings.nested ? "<input type='hidden' id='emailsSelecionados' value='"+text+"' name='"+name+"'/>" : "")
    ...
    return this.each(function(){
      ...
      var field = $('<div></div>')
        ...
        .replaceAll(this);
      if (!settings.nested)
        field.prepend("<input type='hidden' id='emailsSelecionados' value='"+tokens.join(';')+"' name='"+name+"'/>")
